...
For each discovered workflow, scan its nodes for references using the deterministic scanner:

```bash
node n8n-sdlc/scripts/scan-refs.mjs \
  --workflow "{localFilePath}" \
  --mappings n8n-sdlc/config/id-mappings.json
```

**Note:** During initial import, id-mappings.json may not yet have all entries.
For each reference the script reports as `unmapped`:
...
After discovery, once all workflow JSONs are saved locally, build the graph:

```bash
node n8n-sdlc/scripts/build-graph.mjs \
  --workflows "{space-separated list of local workflow JSON paths}" \
  --mappings n8n-sdlc/config/id-mappings.json \
...
- `graph`: each workflow with `calls`, `calledBy`, `type` (agent/tool),
  `isTopLevel`, `isSubAgent`, `isShared`
- `seedOrder`: bottom-up ordering for seeding
- `mermaid`: Mermaid diagram source
- `externalDeps`: external dependencies with referencing workflows

Save `seedOrder` to `metadata.seedOrder` in id-mappings.json.
...
The `mermaid` field from the `build-graph.mjs` output (Step 4) contains the
complete Mermaid diagram source. Wrap it in a Markdown file and save.
...
{mermaid code block from build-graph output}
...
- **No connections** (single workflow with no sub-workflow calls): Diagram will have just the one node
- **Circular references**: Rendered as-is — Mermaid handles cycles gracefully
