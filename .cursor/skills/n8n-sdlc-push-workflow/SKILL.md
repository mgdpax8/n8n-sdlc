---
name: n8n-sdlc-push-workflow
description: Push a local workflow to n8n with drift detection and safety checks. PROD pushes require explicit confirmation. Use when user says "push workflow", "update n8n", "sync to n8n", or "deploy workflow".
---
...
Call the **n8n-sdlc-validate-workflow** skill checks:

1. `n8n-sdlc/config/project.json` exists and is valid
2. `n8n-sdlc/config/id-mappings.json` exists
...
Parse environment from workflow name using `devPrefix` from `n8n-sdlc/config/project.json`:
...
This update will PUBLISH IMMEDIATELY -- changes go live instantly.
...
| Error | Resolution |
|-------|------------|
| Workflow ID not found | Run n8n-sdlc-reserve-workflows skill first |
...
|-----------------|-----|
| project.json missing | Run the n8n-sdlc-getting-started skill |
| id-mappings.json missing | Run the n8n-sdlc-getting-started skill |
| Name doesn't match pattern | Rename workflow to follow convention |
| ID not in mappings | Run n8n-sdlc-reserve-workflows skill |
...
- Does not create new workflows (use n8n-sdlc-reserve-workflows instead)
- Does not transform IDs (use n8n-sdlc-promote-workflow for that)
- Does not push without validation
- Does not push to PROD without confirmation

## Related Skills

- **n8n-sdlc-validate-workflow** - Called automatically before push
- **n8n-sdlc-pull-workflow** - Fetch latest from n8n
- **n8n-sdlc-promote-workflow** - Push DEV to PROD with ID transformation
- **n8n-sdlc-reserve-workflows** - Reserve slots before first push
- **n8n-sdlc-git-sync** - Called automatically after push to commit and push to git
