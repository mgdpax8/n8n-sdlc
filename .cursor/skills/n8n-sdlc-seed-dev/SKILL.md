...
**First, check `metadata.seedOrder` in id-mappings.json.** If present and non-empty,
filter to the workflows being seeded and use that order.

**If seedOrder is missing or empty, compute it with the graph builder:**
...
Parse the JSON output and use the `seedOrder` array. Save the computed order
to `metadata.seedOrder` in id-mappings.json for future runs.
...
- Show `report` to user (name change, ID swap, references reversed,
  credentials mapped)
