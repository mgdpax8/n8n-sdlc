# Skill: Seed DEV from PROD

Transform a PROD workflow into a DEV-ready copy and push it to the DEV slot (reverse promotion).

## When to Use

- After importing existing workflows with import-project skill
- After reserving DEV slots with reserve-workflows skill
...
This is the inverse of the promote-workflow skill.
...
Run the "reserve-workflows" skill to create and claim a DEV slot first.
```
...
2. Update `config/id-mappings.json`:
...
| DEV slot not reserved | Run reserve-workflows first |
...
- `import-project.md` - Run before this to discover and register workflows
- `reserve-workflows.md` - Run before this to create DEV slots
- `promote-workflow.md` - The forward version of this (DEV -> PROD)
- `pull-workflow.md` - Pull latest PROD before seeding if needed
