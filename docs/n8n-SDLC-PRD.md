...
| Environment detection | Auto-detect dev/prod from workflow name prefix | P0 |
| Project validation | Verify workflow belongs to configured project | P1 |

### 5.4 Dev-to-Prod Promotion

| Capability | Description | Priority |
|------------|-------------|----------|
| Name transformation | Change DEV- prefix to PROD- | P0 |
| Workflow ID transformation | Replace dev workflowId references with prod IDs | P0 |
| Credential ID transformation | Replace dev credential IDs with prod IDs (where mapped) | P0 |
| Dependency validation | Verify all referenced workflows have prod slots | P0 |
| Dry-run mode | Preview changes without executing | P1 |
| Audit logging | Record promotion timestamp and details | P1 |

### 5.5 Validation & Safety Checks
...
│   └── DEV-Project-Agent.json
├── tools/                     # Tool workflow JSON files
│   └── DEV-Project-Tool.json
├── docs/
...
### 9.3 Promotion
1. Run "Promote" skill
2. Review transformation summary
3. Confirm promotion
4. Verify in n8n
...
| Promotion | The process of copying a dev workflow to prod with ID transformation |
| MCP | Model Context Protocol - interface for AI to interact with n8n |

### B. Related Documents

- [Billing Bot PRD](../example/docs/Billing%20Bot-PRD.md) - Example project PRD
