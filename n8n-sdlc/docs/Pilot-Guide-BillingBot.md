...
**Alternative for brownfield onboarding:** If you already have workflows in n8n and want to discover them automatically, say **"Get started"** and choose Option 1 (master workflow) or Option 2 (full project pull). The setup wizard handles config creation and then runs discovery automatically. See the **n8n-sdlc-getting-started** skill for details.
...
**Result:** Creates `n8n-sdlc/config/project.json` and `n8n-sdlc/config/id-mappings.json`

### Step 1.2: Verify Configuration

Check that `n8n-sdlc/config/project.json` looks correct:
